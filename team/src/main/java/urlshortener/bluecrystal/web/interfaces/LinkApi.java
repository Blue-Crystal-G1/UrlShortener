...
public interface LinkApi {
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 403, message = "Need to be logged in order to perfom this action", response = ApiErrorResponse.class)})
    @RequestMapping(value = "/link", produces = { "application/json" }, method = RequestMethod.POST)
    ResponseEntity<?> createShortURL(
            @ApiParam(value = "URL to shorten", required = true) @RequestParam("url") String url,
